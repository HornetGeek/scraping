...
def automate_facebook(num_of_shares,keywords,num_of_keyword,account,youtube_link,file):
    ...
    try:
        ...
        if file == "no":
            ...
            time.sleep(5)
            try:
                driver.find_element(By.XPATH, '/html/body/div[1]/div/div[1]/div/div[3]/div/div/div/div[1]/div[1]/div[2]/div/div/div/div/div/div[2]/div/div').click()
            ...
            for link in list_of_groups:
                if num_of_share < int(num_of_shares):
                    driver.get(link)    
                    time.sleep(4)
                    driver.find_element(By.XPATH, '/html').click()
                    time.sleep(3)
                    ...
                    try:
                        driver.find_element(By.XPATH, "//*[contains(text(), 'Write something...')]").click()
                        time.sleep(2)
                    ...
                    text = u"""
                      ...
                      ■ Must Be Residing Within Kzn , Gauteng And Pretoria Regio
            ...
            for link in list_of_groups:
                if num_of_share < int(num_of_shares):
                    ...
                    try:
                        driver.find_element(By.XPATH, "//*[contains(text(), 'Write something...')]").click()
                        #.find_element(By.XPATH, '/html/body/div[1]/div/div[1]/div/div[3]/div/div/div/div[1]/div[1]/div/div[3]/div/div/div[4]/div/div/div[2]/div/div/div/div[1]/div/div/div/div/div[1]/div/div[1]/span').click()
                    except Exception as e :
                        print("Join group")
                        time.sleep(3)
                        join = driver.find_element(By.XPATH, '/html/body/div[1]/div/div[1]/div/div[3]/div/div/div/div[1]/div[1]/div/div[3]/div/div/div[1]/div[2]/div/div/div/div/div[2]/div/div[1]/div/div/div/div/div')
                    ...
                    #driver.execute_script("arguments[0].click();", box_of_post)
                    time.sleep(4)
                    l = driver.find_element(By.XPATH, "/html/body/div[1]/div/div[1]/div/div[4]/div/div/div[1]/div/div[2]/div/div/div/div/div[1]/form/div/div[1]/div/div/div[1]/div/div[2]/div[1]/div[1]/div[1]/div[1]/div/div/div/div/div/div/div").send_keys(youtube_link)
                    #l.send_keys("selenium")
                    time.sleep(2)
                    post_click = driver.find_element(By.XPATH, '/html/body/div[1]/div/div[1]/div/div[4]/div/div/div[1]/div/div[2]/div/div/div/div/div[1]/form/div/div[1]/div/div/div[1]/div/div[3]/div[2]/div/div/div')
                    time.sleep(2)
        ...
        accounts_file = open("accounts_fb.txt",'r').read().splitlines()
        myline= random.choice(accounts_file)
        account = str(myline).split(":")
        try:
            text = "url : {} : error is : {} \n".format(link, e)
        except Exception as e :
            text = "problem on login : {} : error is : {} \n".format(account[0], e)
...
def linkedin(num_of_shares,keywords,num_of_keyword, account,youtube_link,file):
    ...
    try:
        ...
        print("there is error check errors.txt")
        print("we will try with another account")
        accounts_file = open("accounts_linkedin.txt",'r').read().splitlines()
        myline= random.choice(accounts_file)
        account = str(myline).split(":")
        ...
        for li in groups_links:
            if I<num_of_shares:
                ...
                try:
                    ...
                    print("there is error check errors.txt")
                    print("we will try with another account")
                    accounts_file = open("accounts.reddit.txt",'r').read().splitlines()
                    myline= random.choice(accounts_file)
                    account = str(myline).split(":")
        ...
        for li in groups_links:
            if I< num_of_shares:
                try:
                    ...
                    #driver.navigate().refresh()
                    time.sleep(4)
                    #By.CSS_SELECTOR, 'input[placeholder*="some te"]'
                    titl=driver.find_element(By.CSS_SELECTOR, 'textarea[placeholder*="Title"]').send_keys('youtube link')
                    time.sleep(3)
                    driver.find_element(By.CLASS_NAME, "public-DraftEditor-content").send_keys(str(youtube_link))
                    #write=driver.find_element(By.XPATH,'/html/body/div[1]/div/div[2]/div[2]/div/div/div/div[2]/div[3]/div[1]/div[2]/div[4]/div[2]/div[2]/div/div/div[3]/div/div[1]/div/div/div').send_keys(str(youtube_link))
                    time.sleep(5)
                    driver.find_element(By.XPATH, '//*[@id="AppRouter-main-content"]/div/div/div[2]/div[3]/div[1]/div[2]/div[3]/div[3]/div[2]/div[1]/div/div[1]/button').click()
                    #butto=driver.find_element(By.XPATH,'/html/body/div[1]/div/div[2]/div[2]/div/div/div/div[2]/div[3]/div[1]/div[2]/div[4]/div[3]/div[2]/div/div/div[1]/button').click()
                    print("share done")
                    I+=1
                    time.sleep(7)
                    ...
                    with open("errors.txt", "a") as f:
                        f.write(text)
                    driver.close()
                    reddit(num_of_shares,keywords,num_of_keyword, account,youtube_link, file)
...
try:
    while True:
        ...
        if int(num_of_platform) == 1:
            ...
            accounts_file_fb = open("accounts_fb.txt",'r').read().splitlines()
            myline= random.choice(accounts_file_fb)
            account_facebbok = str(myline).split(":")

            accounts_file_linkedin = open("accounts_linkedin.txt",'r').read().splitlines()
            myline= random.choice(accounts_file_linkedin)
            account_linedin = str(myline).split(":")

            accounts_file_reddit = open("accounts.reddit.txt",'r').read().splitlines()
            myline= random.choice(accounts_file_reddit)
            account_reddit = str(myline).split(":")
            face = threading.Thread(target=automate_facebook,args=(num_of_shares,keywords,num_of_keyword,account_facebbok,youtube_link, file))
            face.start()

            linked = threading.Thread(target=linkedin,args=(num_of_shares,keywords,num_of_keyword, account_linedin,youtube_link, file))
            linked.start()

            redditt = threading.Thread(target=reddit,args=(num_of_shares,keywords,num_of_keyword, account_reddit,youtube_link, file))
            redditt.start()
except Exception as e :
    x=0
    while x < 4 :
